...
export async function GET() {
  try {
    ...
    const totalClicks = user.links.reduce((sum: number, link: { clickCount: number }) => sum + link.clickCount, 0)
    
    
    const topLinks = user.links.slice(0, 5)
    
    
    const platformStats = user.links.reduce((acc: Record<string, number>, link: { type: string; clickCount: number }) => {
